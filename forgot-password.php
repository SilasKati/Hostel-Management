...
if($email == "" || $contact == "")
{
	echo "<script>alert('Enter both Email ID and Contact Number.');</script>";
}
else{
$stmt=$mysqli->prepare("SELECT email,contactNo,password FROM userregistration WHERE (email=? && contactNo=?) ");
				...
				{
					echo "<script>alert('Invalid Email ID or Contact No.');</script>";
				}
			}
}
				?>
...
{ ?>
					...
					<?php }  ?>
								<form action="" class="mt" method="post">
									...
									<input type="text" placeholder="Contact no" name="contact" pattern="[0-9]{10}" title="Invalid Number: Enter 10-digit number." class="form-control mb">
									

									<input type="submit" name="login" class="btn btn-primary btn-block" value="Get Password" >
								</form>
							</div>
						</div>
